...
def replace_tags(tree, sentence, xpos=True):
    if tree.is_leaf():
        raise ValueError("Must call replace_tags with non-leaf")

    if xpos:
        tags = [x.xpos for x in sentence.words]
    else:
        tags = [x.upos for x in sentence.words]
    ...
    if any(False for _ in tag_iterator):
...
def retag_trees(trees, pipeline, xpos=True):
    ...
    doc = pipeline(doc)
    new_trees = [replace_tags(tree, sentence, xpos) for tree, sentence in zip(trees, doc.sentences)]
    return new_trees
